    ...
    <AnimatePresence>
      ...
      >
        ...
        <motion.div
          ...
          className="relative bg-surface w-full max-w-4xl max-h-[90vh] overflow-hidden rounded-2xl shadow-2xl"
        >
          {/* Close button - always visible */}
          <button
            onClick={onClose}
            className="absolute top-4 right-4 z-10 w-10 h-10 bg-surface-muted hover:bg-primary-soft
              text-subtle transition-all duration-300 rounded-full flex items-center justify-center cursor-pointer"
          >
...
{/* Title */}
<h2 className="text-3xl md:text-4xl font-bold text-heading leading-tight mb-3">
  {project.title}
</h2>

{/* Subtitle Badge */}
<span className={`${!project.subTitle ? "hidden" : "inline-block bg-primary-soft text-primary text-sm font-semibold px-4 py-1.5 rounded-full tracking-wide"}`}>
  {project.subTitle}
</span>
...
{project.description && (
<div className="mb-8">
  <p className="text-subtle leading-relaxed text-[15px] md:text-base max-w-3xl">
    {project.description}
  </p>
</div>
)}
            ...
            <div className="pt-6 border-t border-soft flex gap-3 flex-wrap">
              {project.github && (
