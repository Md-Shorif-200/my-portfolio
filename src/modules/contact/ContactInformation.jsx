...
const ContactInformation = () => {
  return (
    <div className="flex flex-col gap-2 shadow-xs border border-gray-100 p-4 rounded-xl">
      {contactInfo.map((item) => {
        const Icon = item.icon;

        const content = (
          <div className="flex items-start gap-4 group p-3 -mx-3 rounded-xl hover:bg-black/[0.03] transition-colors duration-200">
            <div
              className="w-11 h-11 shrink-0 rounded-full flex items-center justify-center transition-transform duration-300 group-hover:scale-105"
            ...
            <div className="pt-0.5">
              ...
              <p className="text-black/45 text-sm mt-1">{item.subtitle}</p>
            </div>
          </div>
        );
